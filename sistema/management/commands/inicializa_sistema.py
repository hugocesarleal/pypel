...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        #cria os perrfis de usuario
        perfil_administrador, created = Perfil.objects.get_or_create(nome='Administrador')
        if created:
            self.stdout.write(self.style.SUCCESS(f'Perfil criado: {perfil_administrador.nome}'))

        perfil_estoquista, created = Perfil.objects.get_or_create(nome='Estoquista')
        if created:
        ...
        if not User.objects.filter(email='adm@gmail.com').exists():
            ...
            self.stdout.write(self.style.SUCCESS('Usuário Administrador já existe:'))
